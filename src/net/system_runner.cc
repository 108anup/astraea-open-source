...
pair<string, string> run(const string& filename, const vector<string>& args,
                         const bool read_stdout_until_eof,
                         const bool read_stderr_until_eof,
                         const vector<string>& env, const bool use_environ,
                         const bool path_search) {
  ...
  ChildProcess command_process(
      args[0], [&filename, &args, &env, use_environ, path_search, &stdout_pipe,
                &stderr_pipe]() {
        if (stdout_pipe) {
          stdout_pipe->first.close();
          CheckSystemCall("dup2",
                          dup2(stdout_pipe->second.fd_num(), STDOUT_FILENO));
        }

        if (stderr_pipe) {
          stderr_pipe->first.close();
          CheckSystemCall("dup2",
                          dup2(stderr_pipe->second.fd_num(), STDERR_FILENO));
        }
...
string command_str(const vector<string>& command, const vector<string>& env) {
  ostringstream oss;
